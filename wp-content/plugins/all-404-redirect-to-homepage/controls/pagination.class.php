...
public function pagination($data_source='',$filter='',$rows=10,$parameter='page_num')
	{
...
private function run()
{
	...
	if($this->run_count == 0)
	{
		...
		$result=mysql_query(" select count(*) as cnt from {$this->data_source} {$this->filter} ");
		$res_array=mysql_fetch_array($result);
		$this->table_rows_count = intval($res_array['cnt']);
