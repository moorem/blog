...
function spyropress_get_next_prev_portfolio_link( ) {
    ...
    if ( ! empty( $prev_post ) ) {
        $prev_link = '<a class="portfolio-nav-prev" rel="tooltip" data-original-title="' . __( 'Previous', 'spyroporess' ) . '" href="' . get_permalink( $prev_post->ID ) . '"><i class="icon icon-chevron-left"></i></a>';
    }

    // next post link
    $next_post = get_next_post();
    if ( ! empty( $next_post ) ) {
        $next_link = '<a href="' . get_permalink( $next_post->ID ) . '" class="portfolio-nav-next" rel="tooltip" data-original-title="' . __( 'Next', 'spyropress' ) . '"><i class="icon icon-chevron-right"></i></a>';
    }
