...
function spyropress_row_wrapper( $row_ID, $row ) {
    ...
    if( isset( $container_skin ) && !empty( $container_skin ) ) {
        ...
        if( 'video_section' == $container_skin ) {
            $parallax = ' style="background-image: url(' . $video_poster . ');"';
            
            if( $video_mp4 ) {
                $mp4 = '<source src="' . $video_mp4 . '"  type=\'video/mp4;codecs="avc1.42E01E, mp4a.40.2"\'>';
            }
            
            if( $video_ogg ) {
                $ogg = '<source src="' . $video_ogg . '"  type=\'video/ogg;codecs="theora, vorbis"\'>';
            }
            
            if( $video_mp4 || $video_ogg )
                $video = '<video autoplay loop poster="' . $video_poster . '" width="1920" height="495" data-plugin-options=\'{"features": []}\'>' . $mp4 . $ogg . '</video>';
        }
    }
    				...
    				<span class="pull-right">
                        <span><?php
                            comment_reply_link( array_merge( $args, array(
                                ...
                                'before' => '<i class="icon icon-reply"></i> '
                            ) ) );
                        ?></span>
    				</span>
    			</span>
...
function spyropress_pagination_defaults( $defaults = array() ) {
    ...
    $defaults['container_class'] = 'pagination pagination-lg pull-right';
