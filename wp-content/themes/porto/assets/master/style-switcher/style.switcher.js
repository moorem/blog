...
var styleSwitcher = {
	...
	setColor: function(a) {
		...
		return this.isChanging ? !1 : (b.options.color = a, less.modifyVars({
			...
			skinColor: a
		}), $.cookie("skin", a), void(a == this.container.find("ul[data-type=colors] li:first a").attr("data-color-hex") ? $("h1.logo img").attr("src", theme_settings.assets + "img/logo-default.png") : $("h1.logo img").attr("src", theme_settings.assets + "img/logo.png")))
	},
	setLayoutStyle: function(a, b) {
		...
		c.find("a.active").removeClass("active"), c.find("a[data-layout-type=" + a + "]").addClass("active"), "wide" == a ? (d.hide(), $("body").removeClass("boxed"), $("link[href*='bootstrap-responsive']").attr("href", theme_settings.assets + "css/bootstrap-responsive.css"), $.removeCookie("pattern")) : (d.show(), $("body").addClass("boxed"), $("link[href*='bootstrap-responsive']").attr("href", theme_settings.assets + "css/bootstrap-responsive-boxed.css"), null == $.cookie("pattern") && this.container.find("ul[data-type=patterns] li:first a").click())
	},
	setPattern: function(a) {
		var b = $("body").hasClass("boxed");
		b && $("body").css("background-image", "url(" + theme_settings.assets + "img/patterns/" + a + ".png)"), $.cookie("pattern", a)
	},
	reset: function() {
	...
	getCss: function() {
		...
		a ? (raw = 'body { background-image: url("' + theme_settings.assets + 'img/patterns/' + $.cookie("pattern") + '.png"); }', $("#addBoxedClassInfo").show()) : $("#addBoxedClassInfo").hide(), $("#getCSSTextarea").text(""), $("#getCSSTextarea").text($('style[id^="less:"]').text()), $("#getCSSModal").modal("show"), options = {
			indent: "	",
			autosemicolon: !0
